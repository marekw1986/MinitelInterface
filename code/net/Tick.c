...
// disabled.
static volatile DWORD dwInternalTicks = 0;//0xfffffff0;
...
void TickInit(void)
{
	...
	// Start timer
	T1CONbits.TON = 1;
//	printf("Tick.c init: tps=%llu pc=%lu sys=%lu\r\n",TICKS_PER_SECOND,GetPeripheralClock(),GetSystemClock());
#endif
}
...
{
unsigned int x,a,b;
extern volatile unsigned int uptime;
...
a=30;
	...
	do
	{
		...
		// corner case.
		#if __PIC32_FEATURE_SET__ <= 460 && __PIC32_FEATURE_SET__ > 270
		#warning AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa
			if(((WORD*)vTickReading)[0] == 0xFFFFu)
			{
			//	printf("GetTickCopy:warning  dwTempTicks=%u-1=%u  \r\n",dwTempTicks,dwTempTicks-1);
				dwTempTicks--;
				x++;
			}
	...
	IEC0SET = _IEC0_T1IE_MASK;		// Enable interrupt
	//if(x) printf("GetTickCopy:warning  start=%u dwTempTicks=%u x=%u  \r\n",a,a-x,x);
#endif
}
