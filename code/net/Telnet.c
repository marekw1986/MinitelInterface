...
static ROM BYTE strTitle[]			= "\x1b[2J\x1b[31m\x1b[1m"	// 2J is clear screen, 31m is red, 1m is bold
									  ...
									  "(for this demo, type 'admin' for the login and 'microchip' for the password.)\r\n"
								  	  "Login: ";
// Demo password
static ROM BYTE strPassword[]		= "Password: \xff\xfb\x01";	// DO Suppress Local Echo (stop telnet client from printing typed characters)
// Restore local echo cmd
static ROM BYTE restoreEcho[]       = "\xff\xfc\x01";
...
void TelnetTask(void)
{
    ...
    static BYTE cmdBuffer[MAX_TELNET_CONNECTIONS][MAX_SHELL_CMD_LEN+1];
    static BYTE cmdIndex[MAX_TELNET_CONNECTIONS];
	...
	for(vTelnetSession = 0; vTelnetSession < MAX_TELNET_CONNECTIONS; vTelnetSession++)
	{
		...
		switch(TelnetState)
		{
			...
			case SM_AUTHENTICATED:
				if(TCPIsPutReady(MySocket) < strlenpgm((ROM char*)strDisplay) + strlenpgm((ROM char*)restoreEcho) + 4)
					break;
	
				TCPPutROMString(MySocket, strDisplay);
                TCPPutROMString(MySocket, restoreEcho);
				TelnetState++;
...
//						break;
                        ...
                        case '\n':
                        cmdBuffer[vTelnetSession][cmdIndex[vTelnetSession]] = '\0';
                        cmdIndex[vTelnetSession] = 0;
                        TCPPutString(MySocket, cmdBuffer[vTelnetSession]);
                        TCPPutROMString(MySocket, "\r\n");
                        break;
                        
                        default:
                        cmdBuffer[vTelnetSession][cmdIndex[vTelnetSession]] = i;
                        cmdIndex[vTelnetSession]++;
                        if (cmdIndex[vTelnetSession] > MAX_SHELL_CMD_LEN) cmdIndex[vTelnetSession] = 0;
                        break;
					}
				}
